...
void Player::fightEnemy(int level)
{
  ...
  Enemy thief{"You have encountered a thief!", "You stole the thief's life!",
              "You were shot!", 0};
  Enemy dog{"You have encountered a rabid dog!",
            "You kindly sent the dog to the animal shelter", "You got rabies!",
            0};
  Enemy snake{"You have found a venomous snake!", "You stomped it's head.",
              "You got bit!", 0};

  Enemy easyEnemies[] = {thief, dog, snake};

  Enemy archer{"You have encountered a skilled archer!", "You shot em first!",
               "You got sniped!", 1};
  Enemy swordsman{"You have encountered a swordsman!", "You stabbed his heart!",
                  "You lost your arm.", 1};

  Enemy mediumEnemies[] = {archer, swordsman};

  Enemy gladiator{"You have encountered a gladiator!", "You got em!",
                  "Bro said THIS IS SPARTA and shoved you down a hole.", 3};
  Enemy lion{"You have encountered a lion!",
             "You got ate him for dinner with you family.", "You got eaten!",
             3};

  Enemy hardEnemies[] = {gladiator, lion};

  Enemy lavaMonster{"You have encountered a lava monster!",
                    "You brought him to the ocean.",
                    "Your now part of a volcanoe.", 4};
  Enemy evilWizard{"You have encountered an evil wizard!",
                   "You turned him into a chicken (you went to magic school).",
                   "Your now a ladybug!", 5};

  Enemy harderEnemies[] = {lavaMonster, evilWizard};

  Enemy thanos{"You have encountered Thanos!",
               "You stole his stones and sent "
               "him to the shadow realm",
               "You were snapped into the next dimension (literally).", 10};

  Enemy doofenshmirtz{
      "You have stumbled across LEGENDARY Dr. Heinz Doofenshmirtz!!!!!!!!!!",
      "You found perry and foiled his evil plan! CURSE YOU PERRY THE PLATIPUS!",
      "You were killed by Doofenshmirtz and his Inexplicable Giant Floating "
      "Baby Head-Attract-Inator!!!!! What an honor!",
      10};
  ...
  {
    if (powerDifference + healthDifference > 0)
    {
...
void Player::explore()
{
  ...
  PickUp healthPotion{"You have found a health potion!",
                      25 + (std::rand() % 50), 0};
  ...
  PickUp lootBox{"You found a small a potion chest!", 25 + (std::rand() % 50),
                 10 + std::rand() % 20};

  PickUp goodMiddleItems[] = {healthPotion, steak, powerPotion, spinich,
                              lootBox};

  PickUp peanutButter{"You accidentally age peanut butter, your allergic!!",
                      -25 - (std::rand() % 50), 0};
  PickUp spoiledEgg{"You accidentally ate a spoiled egg.",
                    -10 - (std::rand() % 20), 0};
  PickUp hole{"You sprained you ankle in a hole!", 0, -25 - (std::rand() % 50)};
  PickUp nail{"You broke your pinky nail!", -10 - (std::rand() % 20),
              -10 - (std::rand() % 20)};
  ...
  Cooridinate direction = {float((std::rand() % 3) - 1),
                           float((std::rand() % 3) - 1)};
  ...
  distanceTraveled +=
      std::sqrt(std::pow(displacement.x, 2) + std::pow(displacement.x, 2));
